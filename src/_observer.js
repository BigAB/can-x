...
function observerx(componentClass) {
	...
	componentClass.isCanXReactObserver = true;
...
Observer.propTypes = {
	children: (propValue, key, componentName, location, propFullName) => {
		if (typeof propValue[key] !== 'function')
			return new Error('Invalid prop `' + propFullName + '` of type `' + typeof propValue[key] + '` supplied to' + ' `' + componentName + '`, expected `function`.');
		}
	}
