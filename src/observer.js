...
const reactiveMixin = {
  componentWillMount() {
    ...
    this.render.on("change", (ev, newVal) => {
      this.forceUpdate();
    });
  },
  componentDidMount() {
...
function createObservedClass(componentClass, config = { read: true }) {
  if (!componentClass) {
    throw new Error("Please pass a valid component to 'observer'");
  }
  if (componentClass.isCanXInjector === true) {
    console.warn(
      "CanX observer: You are trying to use 'observer' on a component that already has 'inject'. Please apply 'observer' before applying 'inject'"
    );
  }
